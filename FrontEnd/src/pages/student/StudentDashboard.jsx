...
const MOCK_ACTIVITY_FEED = [
  {
    id: 1,
    type: 'grade',
    message: 'Your assignment "OS Installation" was graded: 92/100',
    time: '1 hour ago',
  },
  {
    id: 2,
    type: 'complete',
    message: 'You completed Module 3: Networking Fundamentals',
    time: '2 hours ago',
  },
  {
    id: 3,
    type: 'announce',
    message: 'Instructor posted: "Lab session moved to Friday 2PM"',
    time: '1 day ago',
  },
  {
    id: 4,
    type: 'complete',
    message: 'You completed all modules in PC Hardware Assembly!',
    time: '2 days ago',
  },
  {
    id: 5,
    type: 'grade',
    message: 'Your quiz "Cable Types" was graded: 88/100',
    time: '3 days ago',
  },
];
...
export default function StudentDashboard() {
  ...
  const completedCourses = MOCK_ENROLLED_COURSES.filter(
    (c) => c.modulesCompleted === c.modulesTotal
  ).length;

  const totalLessonsCompleted = MOCK_ENROLLED_COURSES.reduce(
    (sum, c) => sum + c.modulesCompleted, 0
  );

  const totalLessons = MOCK_ENROLLED_COURSES.reduce(
    (sum, c) => sum + c.modulesTotal, 0
  );
  ...
  const upcomingThisWeek = MOCK_DEADLINES.filter((d) => {
    const diff = Math.ceil((d.dueDate - Date.now()) / 86400000);
    return diff <= 7;
  }).length;
      ...
      <div className="student-stats-grid">
        <div className="student-stat-card student-stat-blue">
          ...
          <div className="sstat-body">
            <div className="sstat-value">{MOCK_ENROLLED_COURSES.length}</div>
        ...
        <div className="student-left-col">
            ...
            <div className="courses-list">
              {MOCK_ENROLLED_COURSES.map((course) => {
                const progress = Math.round(
                  (course.modulesCompleted / course.modulesTotal) * 100
                );
                const isCompleted =
                ...
                return (
                  <div
                    ...
                    tabIndex={0}
                    onKeyDown={(e) =>
                      e.key === 'Enter' && navigate(course.path)
                    }
                  >
                    <div
                    ...
                    <div className="course-info">
                      <div className="course-title-row">
                        ...
                        {isCompleted && (
                          <span className="course-badge-done">✓ Done</span>
                        )}
                      </div>
                      <div className="course-meta">
                        <span className="course-category">{course.category}</span>
                        <span className="course-activity">
                      ...
                      <div className="course-progress-row">
                        ...
                        <span className="course-progress-text">
                          {course.modulesCompleted}/{course.modulesTotal} modules
                        </span>
                      </div>
                    </div>
          ...
          <div className="student-card">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={MOCK_WEEKLY_PROGRESS}>
          ...
          <div className="student-card">
            ...
            <div className="deadlines-list">
              {MOCK_DEADLINES.map((item) => (
                <div
                  key={item.id}
                  className={`deadline-item ${getDeadlineClass(item.dueDate)}`}
                >
